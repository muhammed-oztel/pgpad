...
async fn execute_query_with_results(
    ...
    // Strip trailing semicolons and append FORMAT JSONCompact
    let trimmed = query.trim().trim_end_matches(';');
    let query_with_format = format!("{} FORMAT JSONCompact", trimmed);
    ...
    for chunk in response.data.chunks(batch_size) {
